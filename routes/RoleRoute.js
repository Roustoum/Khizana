...
roleRoute.get('/', authMidleware, async (req, res, next) => {
    ...
    if (!req.user.role.permissions.roles_permissions.view) {
        throw new CustomError("غير مصرح لك بمشاهدة الأدوار", 403);
    }
...
roleRoute.post('/', authMidleware, async (req, res, next) => {
    ...
    if (!req.user.role.permissions.roles_permissions.create) {
        throw new CustomError("غير مصرح لك بإنشاء الأدوار", 403);
    }
...
roleRoute.put("/:id", authMidleware, async (req, res, next) => {
    ...
    if (!req.user.role.permissions.roles_permissions.edit) {
        throw new CustomError("غير مصرح لك بتعديل الأدوار", 403);
    }
...
roleRoute.delete("/:id", authMidleware, async (req, res, next) => {
    ...
    if (!req.user.role.permissions.roles_permissions.delete) {
        throw new CustomError("غير مصرح لك بحذف الأدوار", 403);
    }
    const role = await Role.findById(req.params.id);
    if (!role) {
        throw new CustomError("الدور المطلوب غير موجود", 404);
    ...
    if (role.immutable) {
        throw new CustomError("لا يمكن حذف هذا الدور", 403);
    }
